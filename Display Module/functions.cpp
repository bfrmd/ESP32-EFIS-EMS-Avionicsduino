...
void initMenu()
{
  ...
  Serial.println();
  Serial.println("Avionicsduino ESP32 EFIS-EMS Display Module V 1.1");
...
void OnDataRecv(const uint8_t* mac, const uint8_t* incomingData, int len)
{
  ...
  // Here we need to capture the extreme values ​​of AccZ, then filter AccZ
  if ((fromRDAMdata.AccZ > accZmax) && (fromRDAMdata.groundSpeedGNSS >= 20))  // Transient G-forces related to potential jolts during slow taxiing (<20Km/h) are not taken into account.
    {
  ...
  fromRDAMdata.AccZ = iirFilter(previousFilteredAccZValue, fromRDAMdata.AccZ, 0.1);  // Moderate filtering
...
void displayFuel()
{
  ...
  tft.setCursor(150, 385);
  tft.printf("Fuel G %02u-%02u", fromRDAMdata.lowerLimit, fromRDAMdata.upperLimit);
}

void displayCurrent()
{
  tft.setCursor(170, 410);
  tft.printf("Ibat %4.1f", fromRDAMdata.iBat);
  tft.setCursor(170, 430);
  tft.printf("Ibus %4.1f", fromRDAMdata.iBus);
}
